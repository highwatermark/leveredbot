...
k-NN classification on a 10-feature vector computed from daily bar data.
...
Features:
  ...
  6. distance_from_20ma   - (close-MA20)/MA20
  7. distance_from_50ma   - (close-MA50)/MA50
  8. distance_from_200ma  - (close-MA200)/MA200
  9. rsi_14               - 14-day RSI
  10. momentum_10         - 10-day rate of change

Usage:
    knn = KNNSignal()
    knn.fit_from_bars(qqq_bars)  # train on historical data
    result = knn.predict(qqq_bars)  # predict next-day direction
"""
...
class FeatureCalculator:
    """Compute the 10-feature vector from daily bar data."""
    ...
    @staticmethod
    def compute_features(bars: list[dict], index: int) -> np.ndarray | None:
        """
        Compute 10-feature vector at a given bar index.

        Args:
            bars: List of bar dicts with keys: open, high, low, close
            index: Index of the bar to compute features for (needs 200+ bars before it)

        Returns:
            numpy array of 10 features, or None if insufficient data
        """
        if index < 200:
            return None
        ...
        # Feature 8: distance from 200-MA
        if len(closes) >= 200:
        ...
        # Feature 10: 10-day momentum (rate of change)
        if index >= 10:
...
class KNNSignal:
    ...
    using distance-weighted k-NN on a 10-feature vector.
    """
    ...
    def fit_from_bars(self, bars: list[dict]) -> bool:
        """
        ...
        Args:
            bars: List of bar dicts with open/high/low/close
        ...
        for i in range(200, len(bars) - 1):
            features = calc.compute_features(bars, i)
            if features is None:
                continue
        ...
        logger.info(f"k-NN model trained: {len(X)} samples, k={self.n_neighbors}")
        return True
    ...
    def predict(self, bars: list[dict]) -> dict:
        ...
        calc = FeatureCalculator()
        features = calc.compute_features(bars, len(bars) - 1)
        if features is None:
    ...
    def save(self, path: Path | str) -> None:
        ...
        logger.info(f"k-NN model saved to {path}")
    ...
    def load(self, path: Path | str) -> bool:
        ...
        try:
            ...
            self.is_fitted = True
            logger.info(f"k-NN model loaded from {path} ({self.training_samples} training samples)")
            return True
        except Exception as e:
            logger.error(f"Failed to load model: {e}")
            return False

    def get_similar_days(self, bars: list[dict], n: int = 5) -> list[dict]:
        """
